...
///// PRIVATE METHODS /////
    ...
    var _qz = {
        ...
        websocket: {
            ...
            connectConfig: {
                host: ["localhost"], 					//hosts QZ Tray can be running on
                ...
                port: {
                    secure: [8181, 8282],   			//list of secure ports QZ Tray could be listening on
                    insecure: [8182, 8283], 			//list of insecure ports QZ Tray could be listening on
            ...
            setup: {
                ...
                findConnection: function(config, resolve, reject) {
                    var deeper = function() {
                        ...
                        if ((config.usingSecure && config.port.portIndex >= config.port.secure.length)
                            ...
                            if (config.hostIndex >= config.host.length - 1) {
                                ...
                                reject(new Error("Não foi possível estabelecer uma conexão com o driver da impressora. " +
                                		"Verifique se o driver foi instalado corretamente e está ativo nesta máquina. " +
                                		"Um ícone verde com o título \"QZ Tray\" deve ser exibido na barra de ferramentas. "));
                                return;
                            } else {
                                config.hostIndex++;
                    ...
                    try {
                        _qz.log.trace("Tentando conectar", address);
                    ...
                    if (_qz.websocket.connection != null) {
                        ...
                        _qz.websocket.connection.onopen = function(evt) {
                            _qz.log.trace(evt);
                            _qz.log.info("Conexão estabelecida com o driver da impressora em " + address);
                        ...
                        reject(new Error("Não foi possível criar uma conexão via websocket com a impressora"));
                    }
                },
                ...
                openConnection: function(openPromise) {
                    ...
                    _qz.websocket.connection.onclose = function(evt) {
                        ...
                        for(var uid in _qz.websocket.pendingCalls) {
                            if (_qz.websocket.pendingCalls.hasOwnProperty(uid)) {
                                _qz.websocket.pendingCalls[uid].reject(new Error("A conexão foi fechada antes da resposta ser recebida"));
                            }
                        }
                    };
                    ...
                    _qz.websocket.connection.sendData = function(obj) {
                        _qz.log.trace("Preparando objeto para o websocket", obj);
                        ...
                        try {
                            if (obj.call != undefined && obj.signature == undefined) {
                                ...
                                _qz.security.callSign(_qz.tools.hash(_qz.tools.stringify(signObj))).then(function(signature) {
                                    _qz.log.trace("Assinatura para chamada", signature);
                                ...
                                _qz.log.trace("Assinatura para chamada", obj.signature);
                    ...
                    _qz.websocket.connection.onmessage = function(evt) {
                        ...
                        if (returned.uid == null) {
                            if (returned.type == null) {
                                ...
                                _qz.websocket.connection.close(4003, "Conectado com uma versão incopatível do driver da impressora");
                                ...
                                switch(returned.type) {
                                    ...
                                    case _qz.streams.hid:
                                        ...
                                        break;
                                    default:
                                        _qz.log.warn("Não foi possível determinar o tipo da stream para callback", returned);
                                        break;
                                }
                            }
                        ...
                        _qz.log.trace("Resposta recebida do websocket", returned);

                        var promise = _qz.websocket.pendingCalls[returned.uid];
                        if (promise == undefined) {
                            _qz.log.warn('Nenhuma promessa encontrada para resposta');
                        } else {
    ...
    return {
        ...
        websocket: {
            ...
            connect: function(options) {
                return _qz.tools.promise(function(resolve, reject) {
                    if (qz.websocket.isActive()) {
                        reject(new Error("Já existe uma conexão com o driver aberta"));
                        return;
                    } else if (_qz.websocket.connection != null) {
                        reject(new Error("A tentativa de conexão atual ainda não retornou"));
                        return;
                    }

                    if (!_qz.tools.ws) {
                        reject(new Error("Este navegador não suporta WebSocket. Por favor use outro browser."));
                        return;
            ...
            disconnect: function() {
                return _qz.tools.promise(function(resolve, reject) {
                    if (qz.websocket.isActive()) {
                        ...
                        reject(new Error("Sem conexão aberta com o driver da impressora"))
                    }
                });
            },
